...
async function migrate() {
  ...
  for (const team of teams) {
    ...
    teamMap[team.name] = data[0].id
    console.log(`  ✓ ${team.name} (${team.leader})`)
  }
  ...
  for (const kpi of kpis) {
    ...
    if (teamId) {
      ...
      const data = await res.json()
      kpiMap[kpi.name] = data[0].id
      console.log(`  ✓ ${kpi.team} - ${kpi.name}`)
    }
  }
  ...
  for (const report of reports) {
    ...
    if (teamId && kpiId) {
      ...
      if (res.ok) console.log(`  ✓ ${report.team} - ${report.kpi} (${report.report_date})`)
    }
  }

  console.log('\n=== 마이그레이션 완료! ===')
  console.log(`팀: ${teams.length}개`)
  console.log(`KPI: ${kpis.length}개`)
