...
const getRateBgClass = (rate: number) => {
  if (rate >= 100) return 'bg-green-900/30 text-green-600'
  if (rate >= 70) return 'bg-yellow-900/30 text-yellow-600'
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden mb-8">
          ...
          <div className="p-5">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-white mb-1">주간 회의 요약</h1>
                <div className="flex items-center gap-3">
                  <button
                    onClick={goToPrevWeek}
                    className="no-print p-1 rounded hover:bg-gray-800 transition"
                    aria-label="이전 주"
                  >
                  ...
                  {weekOffset !== 0 && (
                    <button
                      ...
                      className="no-print text-sm text-blue-600 hover:text-blue-400 underline ml-2"
                    >
                      이번 주로
                    </button>
                  )}
        ...
        {loading ? (
          ...
          <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 p-16 text-center">
            ...
            <p className="text-gray-400 text-xl font-medium mb-2">이번 주 보고서가 없습니다</p>
            <p className="text-gray-400 text-sm">보고서를 입력하면 요약이 자동 생성됩니다</p>
          </div>
        ) : (
          <>
            {/* 전체 평균 달성률 */}
            <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden mb-8 print-break-inside-avoid">
              <div className="h-1 bg-blue-600"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              {/* 팀 순위 테이블 */}
              <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden print-break-inside-avoid">
                <div className="h-1 bg-blue-600"></div>
                <div className="p-5">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-700">
                        <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider py-2 px-2">순위</th>
                        <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider py-2 px-2">팀</th>
                        <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wider py-2 px-2">KPI</th>
                        <th className="text-right text-xs font-semibold text-gray-400 uppercase tracking-wider py-2 px-2">평균 달성률</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-700">
                      {teamRankings.map((team, idx) => (
                        <tr key={team.teamName} className="hover:bg-gray-700">
                          ...
                          <td className="py-3 px-2 text-sm text-gray-400 text-right">{team.count}개</td>
                  ...
                  {missingTeams.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {missingTeams.map(t => {
                        ...
                        return (
                          <div key={t.id} className="flex items-center justify-between bg-red-900/20 border border-red-700 rounded-lg px-4 py-3">
                            <div>
                              <p className="text-sm font-medium text-red-400">{t.name}</p>
                              <p className="text-xs text-red-500">팀장: {t.leader}</p>
                            </div>
                            <span className="text-xs font-semibold text-red-600 bg-red-900/30 px-2 py-1 rounded-full">
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-3">
                    {kpiRankings.best.map((r, idx) => (
                      <div key={r.id} className="flex items-center gap-3 bg-green-900/20 border border-green-700 rounded-lg px-4 py-3">
                        <span className="text-lg font-bold text-green-400 w-6 text-center">{idx + 1}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-white truncate">{r.kpi_name}</p>
                  ...
                  {kpiRankings.worst.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {kpiRankings.worst.map((r, idx) => (
                        <div key={r.id} className="flex items-center gap-3 bg-red-900/20 border border-red-700 rounded-lg px-4 py-3">
                        ...
                        <div className="space-y-2 ml-4">
                          {group.items.map((item, idx) => (
                            <div key={idx} className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3">
                              ...
                              <p className="text-sm text-gray-200 whitespace-pre-wrap">{item.action}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="space-y-5">
                    {issuesByTeam.map(group => (
                      <div key={group.teamName}>
                        ...
                        <div className="space-y-2 ml-4">
                          {group.items.map((item, idx) => (
                            <div key={idx} className="bg-red-900/20 border border-red-700 rounded-lg px-4 py-3">
