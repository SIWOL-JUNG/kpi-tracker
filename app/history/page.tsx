...
// PDCA 필드 정의
const PDCA_FIELDS = [
  { key: 'strategy', label: '전략', color: 'bg-blue-900/20 border-blue-700' },
  { key: 'plan', label: 'Plan', color: 'bg-indigo-50 border-indigo-200' },
  { key: 'do_action', label: 'Do', color: 'bg-green-900/20 border-green-700' },
  { key: 'check_result', label: 'Check', color: 'bg-yellow-900/20 border-yellow-200' },
...
const getAchievementBg = (rate: number | undefined): string => {
  if (rate === undefined || rate === null) return 'bg-gray-800 text-gray-400'
      ...
      <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden mb-8">
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">KPI 히스토리</h1>
          ...
          {reports.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-300 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors print:hidden"
            >
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative">
              <label className="block text-sm font-medium text-gray-300 mb-1">팀</label>
              <div className="relative">
                <select
                  value={selectedTeamId}
                  onChange={e => setSelectedTeamId(e.target.value)}
                  className="w-full appearance-none bg-gray-900 border-2 border-gray-700 rounded-lg px-4 py-3 pr-10 text-base font-medium text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                >
            ...
            <div className="relative">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-gray-900 border-2 border-gray-700 rounded-lg px-4 py-3 pr-10 text-base font-medium text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:bg-gray-800/50 disabled:text-gray-400"
                >
                  <option value="">
        ...
        <>
          ...
          {reports.length === 0 && (
            <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden">
              <div className="h-1 bg-blue-600" />
              <div className="p-12 text-center">
                <p className="text-gray-400">이 KPI에 대한 보고서가 없습니다</p>
              </div>
            </div>
          )}
          ...
          {timelineReports.length > 0 && (
            <div className="bg-gray-900 rounded-xl shadow border-2 border-gray-700 overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="relative">
                  {/* 타임라인 세로선 */}
                  <div className="absolute left-[72px] top-0 bottom-0 border-l-2 border-gray-700" />
                      ...
                      return (
                        <div key={report.id} className="relative pl-24">
                          ...
                          <div className="absolute left-0 top-0 w-16 text-right">
                            <span className="text-xs font-semibold text-gray-400 bg-gray-800 rounded px-1.5 py-0.5">
                          ...
                          <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-4">
                            ...
                            <div className="flex flex-wrap gap-4 text-xs text-gray-400 mb-3">
                              {report.monthly_target !== undefined && report.monthly_target !== null && (
                                <span>월 목표: <strong className="text-gray-300">{report.monthly_target.toLocaleString()}</strong></span>
                              )}
                              {report.weekly_achievement !== undefined && report.weekly_achievement !== null && (
                                <span>주간 실적: <strong className="text-gray-300">{report.weekly_achievement.toLocaleString()}</strong></span>
                              )}
                              {report.monthly_cumulative !== undefined && report.monthly_cumulative !== null && (
                                <span>월 누적: <strong className="text-gray-300">{report.monthly_cumulative.toLocaleString()}</strong></span>
                              )}
                            </div>
                            ...
                            {/* 이슈 카드 */}
                            {report.issue && (
                              <div className="mt-2 rounded-md border p-3 bg-red-900/20 border-red-700">
